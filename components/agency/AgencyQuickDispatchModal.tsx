...
import { agencyOperationsService } from '@/lib/services/agencyOperationsService';
...
export default function AgencyQuickDispatchModal({
  open, mission, deliverers, vehicles, onClose, onAssigned,
}: Props) {
  ...
  const availableDeliverers = branchDeliverers.filter(d =>
  ...
  const handleAssign = async () => {
    ...
    setSubmitting(true);
    try {
      await agencyOperationsService.quickAssignMission(mission.id, delivererId, vehicleId);
    ...
    <div className="fixed inset-0 z-[100] bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            <p className="text-xs text-gray-400">Dispatch agence</p>
        ...
        <div className="p-5 space-y-4">
          <p className="text-sm text-gray-600">
            {mission.branchName} · {mission.recipientName}
          </p>
          <div>
