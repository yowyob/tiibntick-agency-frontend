...
import {
  AlertTriangle, Package, MapPin, UserX, ChevronRight, Zap,
} from 'lucide-react';
import type { BranchAction } from '@/lib/branch/actionQueue';

const ICONS: Record<BranchAction['type'], React.ElementType> = {
...
const PRIORITY_STYLES: Record<BranchAction['priority'], string> = {
...
export default function BranchActionQueue({ actions, onDispatch, onEscalate, maxItems = 8 }: Props) {
  ...
  if (actions.length === 0) {
    return (
      <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-5 py-4 flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-emerald-800">Antenne sous contrôle</p>
          <p className="text-xs text-emerald-600">Aucune action urgente pour le moment.</p>
        </div>
      </div>
    );
  }
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {action.missionId && onEscalate && action.type === 'DELIVERER_OFFLINE_ON_MISSION' && (
                  <button
                    type="button"
                    onClick={() => onEscalate(action.missionId!)}
                    className="text-[11px] font-bold text-white bg-red-500 hover:bg-red-600 px-2.5 py-1 rounded-lg"
                  >
                    Escalader
                  </button>
                )}
